...
### 7. Prefer Native Screenshot Crops For Desktop Chrome And Small UI Controls

当参考图本身包含桌面应用外壳、系统级窗口按钮、会话工具栏小图标、滚动条、禁用态按钮、极小的分隔符或状态指示器时，优先判断这些内容是不是“截图原生素材”，而不是默认全部重画。
